...
members = [
    attr for attr in dir(
        CONSTANTS.RAID_TIER) if not callable(
            getattr(
                CONSTANTS.RAID_TIER,
...
def load_url(fight_id):
    ...
    driver.get(
        f"https://www.fflogs.com/zone/rankings/44#boss={fight_id.value}&metric=dps&class=Global&spec={CONSTANTS.JOB.value}")
    cookies = driver.find_element(
        by=By.XPATH,
        value="/html/body/div[2]/div/div/div/div[2]/div/button[2]/span")
    sleep(1)
    cookies.click()
    top_player = driver.find_element(
        by=By.XPATH,
        value="/html/body/div[3]/div[3]/div[2]/div[2]/div/div[4]/div[2]/table/tbody/tr[1]/td[2]/div/div[1]/a[1]")
    sleep(1)
    top_player.click()
    ...
    top_player = driver.find_element(
        by=By.XPATH,
        value="/html/body/div/div[2]/div[2]/div/div[1]/div/a[1]/span[1]")
    top_player.click()
    sleep(2)
...
def create_single_file(fight_name, override=False):
    if os.path.exists(
            f"{CONSTANTS.XIV_PATH}/jobs/{CONSTANTS.JOB.__class__.__name__}/{CONSTANTS.JOB.name}/{fight_name}.txt") and not override:
    ...
    with open(f"{action_output_path}/{fight_name}.txt", 'w+', encoding="utf-8") as file:
        for line in actions:
            file.write(f"{line}\n")
    actions.clear()
...
def create_raid_tier(force_refresh=False):
    amount_of_fights = (
        CONSTANTS.RAID_TIER[members[-1]].value + 1) - CONSTANTS.RAID_TIER[members[0]].value
    for i in range(amount_of_fights):
        create_single_file(members[i], force_refresh)
